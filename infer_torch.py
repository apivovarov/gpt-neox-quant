...
def predict(input_ids, attention_mask):
  ...
  return next_token_logits.cpu().numpy()

#input_ids = torch.tensor([tokens[:A_tokens] + [50256]*F_tokens], dtype=torch.int64).to(device)
input_ids = torch.tensor([tokens[:N_tokens]], dtype=int_dtype).to(device)
attention_mask = torch.tensor([[1]*A_tokens+[0]*F_tokens], dtype=dtype).to(device)
output = predict(input_ids, attention_mask)
